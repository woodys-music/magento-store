    ...
    extends Ess_M2ePro_Controller_Adminhtml_Common_MainController
{
    ...
    protected function _initAction()
    {
        ...
        $this->getLayout()->getBlock('head')
            ...
            ->addJs('M2ePro/Common/Buy/Listing/SettingsHandler.js')
    ...
    public function createListingAction()
    {
        ...
        if ($isEmptyListing == 1) {
            if ($this->getRequest()->getParam('back') == 'list') {
                $backUrl = $this->getUrl('*/adminhtml_common_listing/index', array('tab' => 'buy'));
            } else {
    ...
    public function addStepOne()
    {
        ...
        $this->_initAction()
            ->_addContent($this->getLayout()->createBlock('M2ePro/adminhtml_common_buy_listing_add_stepOne'))
            ->renderLayout();
    }
    ...
    public function addStepFour()
    {
        ...
        if ($temp['source_products'] == Ess_M2ePro_Model_Listing::SOURCE_PRODUCTS_CUSTOM) {
            $blockContent = $this->getLayout()->createBlock('M2ePro/adminhtml_common_buy_listing_add_stepFourProduct');
        } else if ($temp['source_products'] == Ess_M2ePro_Model_Listing::SOURCE_PRODUCTS_CATEGORIES) {
            $blockContent = $this->getLayout()->createBlock('M2ePro/adminhtml_common_buy_listing_add_stepFourCategory');
        } else {
            $this->_redirect('*/*/add',array('step'=>'1','clear'=>'yes'));
            return;
        }
        ...
        // Set rule model
        // ---------------------------
        $this->setRuleData('buy_rule_add_listing_product');
        // ---------------------------
    ...
    public function addStepFive()
    {
        ...
        // Set rule model
        // ---------------------------
        $this->setRuleData('buy_rule_add_listing_product_categories');
        // ---------------------------
        ...
        $this->_initAction()
            ->_addContent($this->getLayout()->createBlock('M2ePro/adminhtml_common_buy_listing_add_StepFive'))
            ->renderLayout();
    }
    ...
    public function editAction()
    {
        ...
        $additionalData = array(
            ...
            'shipping_standard_mode' => $model->getShippingStandardMode(),
            'shipping_standard_value' => $model->getData('shipping_standard_value'),
            'shipping_standard_custom_attribute' =>
                                                $model->getData('shipping_standard_custom_attribute'),

            'shipping_expedited_mode' => $model->getShippingExpeditedMode(),
            'shipping_expedited_value' => $model->getData('shipping_expedited_value'),
            'shipping_expedited_custom_attribute' =>
                                                $model->getData('shipping_expedited_custom_attribute'),

            'shipping_one_day_mode' => $model->getShippingOneDayMode(),
            'shipping_one_day_value' => $model->getData('shipping_one_day_value'),
            'shipping_one_day_custom_attribute' =>
                                                $model->getData('shipping_one_day_custom_attribute'),

            'shipping_two_day_mode' => $model->getShippingTwoDayMode(),
            'shipping_two_day_value' => $model->getData('shipping_two_day_value'),
            'shipping_two_day_custom_attribute' =>
                                                $model->getData('shipping_two_day_custom_attribute'),
        );

        Mage::helper('M2ePro/Data_Global')->setValue('temp_data', array_merge($model->getData(), $additionalData));

        $this->_initAction();
        $this->_addLeft($this->getLayout()->createBlock('M2ePro/adminhtml_common_buy_listing_edit_tabs'));
    ...
    public function productAction()
    {
        ...
        $tempData['component_mode'] = Ess_M2ePro_Helper_Component_Buy::NICK;
        ...
        // Set rule model
        // ---------------------------
        $this->setRuleData('buy_rule_listing_product');
        // ---------------------------
        ...
        $this->_initAction()
            ->_addContent($this->getLayout()->createBlock('M2ePro/adminhtml_common_buy_listing_product'))
            ->renderLayout();
    }
    ...
    public function categoryProductAction()
    {
        ...
        if (!is_null($addProducts)) {
            ...
            // Set rule model
            // ---------------------------
            $this->setRuleData('buy_rule_listing_product_categories');
            // ---------------------------
            ...
            $this->_initAction();
            $this->_addContent($this->getLayout()->createBlock('M2ePro/adminhtml_common_buy_listing_product'));
            $this->renderLayout();
        ...
        $listingData['component_mode'] = Ess_M2ePro_Helper_Component_Buy::NICK;
        ...
        $blockContent = $this->getLayout()->createBlock(
            'M2ePro/adminhtml_common_buy_listing_product_category'
        );
    ...
    public function productGridAction()
    {
        ...
        $response = $this->loadLayout()->getLayout()
            ->createBlock('M2ePro/adminhtml_common_buy_listing_product_grid')->toHtml();
        $this->getResponse()->setBody($response);
    }
    ...
    protected function processConnector($action, array $params = array())
    {
        ...
        if ($result == Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_ERROR) {
            return json_encode(
                array('result'=>'error','action_id'=>$actionId,'is_processing_items'=>$isProcessingItems)
            );
        }

        if ($result == Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_WARNING) {
            return json_encode(
                array('result'=>'warning','action_id'=>$actionId,'is_processing_items'=>$isProcessingItems)
            );
        }

        if ($result == Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_SUCCESS) {
            return json_encode(
