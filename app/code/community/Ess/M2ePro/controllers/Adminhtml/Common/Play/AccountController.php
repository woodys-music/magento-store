...
class Ess_M2ePro_Adminhtml_Common_Play_AccountController
    extends Ess_M2ePro_Controller_Adminhtml_Common_MainController
{
    ...
    protected function _initAction()
    {
        $this->loadLayout()
            ...
            ->_title(Mage::helper('M2ePro')->__('Play.com Accounts'));

        $this->getLayout()->getBlock('head')
            ->setCanLoadExtJs(true)
            ->addJs('M2ePro/Common/Play/AccountHandler.js');
    ...
    public function editAction()
    {
        ...
        $model = Mage::helper('M2ePro/Component_Play')->getModel('Account')->load($id);
        ...
        $this->_initAction()
            ->_addLeft($this->getLayout()->createBlock('M2ePro/adminhtml_common_play_account_edit_tabs'))
            ->_addContent($this->getLayout()->createBlock('M2ePro/adminhtml_common_play_account_edit'))
            ->renderLayout();
    }
    ...
    public function saveAction()
    {
        ...
        $temp = Ess_M2ePro_Model_Play_Account::OTHER_LISTINGS_MAPPING_GENERAL_ID_MODE_CUSTOM_ATTRIBUTE;
        ...
        $temp1 = Ess_M2ePro_Model_Play_Account::OTHER_LISTINGS_MAPPING_SKU_MODE_DEFAULT;
        $temp2 = Ess_M2ePro_Model_Play_Account::OTHER_LISTINGS_MAPPING_SKU_MODE_CUSTOM_ATTRIBUTE;
        $temp3 = Ess_M2ePro_Model_Play_Account::OTHER_LISTINGS_MAPPING_SKU_MODE_PRODUCT_ID;
        if (isset($tempData['mapping_sku_mode']) &&
            ...
            $temp = Ess_M2ePro_Model_Play_Account::OTHER_LISTINGS_MAPPING_SKU_MODE_CUSTOM_ATTRIBUTE;
            if ($tempData['mapping_sku_mode'] == $temp) {
                $mappingSettings['sku']['attribute'] = (string)$tempData['mapping_sku_attribute'];
            }
        }

        $temp1 = Ess_M2ePro_Model_Play_Account::OTHER_LISTINGS_MAPPING_TITLE_MODE_DEFAULT;
        $temp2 = Ess_M2ePro_Model_Play_Account::OTHER_LISTINGS_MAPPING_TITLE_MODE_CUSTOM_ATTRIBUTE;
...
//            'customer_created',
        ...
        //--------------------
        $temp = Ess_M2ePro_Model_Play_Account::MAGENTO_ORDERS_INVOICE_MODE_YES;
        $data['magento_orders_settings']['invoice_mode'] = $temp;
        $temp = Ess_M2ePro_Model_Play_Account::MAGENTO_ORDERS_SHIPMENT_MODE_YES;
        $data['magento_orders_settings']['shipment_mode'] = $temp;

        $temp = Ess_M2ePro_Model_Play_Account::MAGENTO_ORDERS_STATUS_MAPPING_MODE_CUSTOM;
        if (!empty($data['magento_orders_settings']['status_mapping']['mode']) &&
            $data['magento_orders_settings']['status_mapping']['mode'] == $temp) {

            $temp = Ess_M2ePro_Model_Play_Account::MAGENTO_ORDERS_INVOICE_MODE_NO;
            if (!isset($post['magento_orders_settings']['invoice_mode'])) {
                $data['magento_orders_settings']['invoice_mode'] = $temp;
            }
            $temp = Ess_M2ePro_Model_Play_Account::MAGENTO_ORDERS_SHIPMENT_MODE_NO;
        ...
        // Add or update model
        //--------------------
        $model = Mage::helper('M2ePro/Component_Play')->getModel('Account');
            ...
            if (!$accountObj->isLockedObject('server_synchronize')) {
                ...
                if (!$isEdit) {
                    ...
                    if (!empty($post['password'])) {
                        $newData['password'] = $post['password'];
                    }
    ...
    public function checkAuthAction()
    {
        ...
        $panelMode = $this->getRequest()->getParam('panel_mode',Ess_M2ePro_Model_Play_Account::PANEL_MODE_TRADE);
