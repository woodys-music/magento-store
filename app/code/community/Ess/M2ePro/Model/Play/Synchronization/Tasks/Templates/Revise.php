...
class Ess_M2ePro_Model_Play_Synchronization_Tasks_Templates_Revise
    ...
    private function executeTotal()
    {
        ...
        if ($collection->count() < $itemsPerCycle) {
            Mage::helper('M2ePro/Module')->getSynchronizationConfig()->setGroupValue(
                '/play/templates/revise/total/','end_date', Mage::helper('M2ePro')->getCurrentGmtDate()
            );
        ...
        Mage::helper('M2ePro/Module')->getSynchronizationConfig()->setGroupValue(
            '/play/templates/revise/total/','last_listing_product_id', $lastListingProduct
        );
