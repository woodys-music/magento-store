...
class Ess_M2ePro_Model_Play_Listing_Product extends Ess_M2ePro_Model_Component_Child_Play_Abstract
{
    ...
    public function dbSelectModifier(Varien_Db_Select $dbSelect) {
        ...
        $dbSelect->join(
            array('plp' => Mage::getResourceModel('M2ePro/Play_Listing_Product')->getMainTable()),
            '`lp`.`id` = `plp`.`listing_product_id`',
            array()
        );

        $dbSelect->where(
            '`plp`.`is_variation_product` = '.self::IS_VARIATION_PRODUCT_NO.
            ' OR ('.
                '`plp`.`is_variation_product` = '.self::IS_VARIATION_PRODUCT_YES.
                ' AND `plp`.`is_variation_matched` = '.self::IS_VARIATION_MATCHED_YES.
            ')'
        );
