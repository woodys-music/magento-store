...
abstract class Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester
    extends Ess_M2ePro_Model_Connector_Server_Buy_Requester
{
    ...
    protected $status = Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_SUCCESS;
    ...
    {
        $this->setStatus(Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_SUCCESS);
        ...
        if (isset($tempMessages) && is_array($tempMessages) && count($tempMessages) > 0) {
            $this->setStatus(Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_ERROR);
        }
                                                  ...
                                                  $isListingMode = true)
    {
        ...
        switch ($type) {
            ...
            case Ess_M2ePro_Model_Log_Abstract::TYPE_WARNING:
                    $this->setStatus(Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_WARNING);
                break;
    ...
    protected function setStatus($status)
    {
        if (!in_array($status,array(
            Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_ERROR,
            Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_WARNING,
            Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_SUCCESS))) {
            return;
        }

        if ($status == Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_ERROR) {
            $this->status = Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_ERROR;
            return;
        }

        if ($this->status == Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_ERROR) {
            return;
        }

        if ($status == Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_WARNING) {
            $this->status = Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_WARNING;
            return;
        }

        if ($this->status == Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_WARNING) {
            return;
        }

        $this->status = Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_SUCCESS;
    }
        ...
        foreach ($listingsProducts as $key => $listingProduct) {
            ...
            if (!in_array($this->getActionIdentifier(),array('stop','stop_and_remove'))) {
                if ($listingProduct->getChildObject()->isVariationProduct() &&
                    ...
                    // ->__('You have to select variation first.');
