...
class Ess_M2ePro_Model_Connector_Server_Play_Dispatcher
{
    //####################################
     ...
     * @return Ess_M2ePro_Model_Connector_Server_Play_Requester|Ess_M2ePro_Model_Connector_Server_Play_Abstract
     */
    ...
    {
        $className = empty($ormPrefixToConnector) ? 'Ess_M2ePro_Model_Connector_Server_Play' : $ormPrefixToConnector;
                                     ...
                                     $ormPrefixToConnector = NULL)
    {
        if (is_int($marketplace) || is_string($marketplace)) {
            $marketplace = Mage::helper('M2ePro/Component_Play')->getCachedObject('Marketplace',(int)$marketplace);
        }

        if (is_int($account) || is_string($account)) {
            $account = Mage::helper('M2ePro/Component_Play')->getCachedObject('Account',(int)$account);
        }
    ...
    public function processVirtualRequester($entity, $type, $name,
                                            $responserModel = 'Play_VirtualResponser',
