...
// ->__('UPC/EAN has a wrong format. Please check Search Settings for ASIN / ISBN  in Listing -> Channel Settings.');
    ...
    private function addSkusToQueue($skus)
    {
        $lockItem = Mage::getModel('M2ePro/LockItem')->load(
            'amazon_list_skus_queue_' . $this->marketplace->getId() . '_' . $this->account->getId(),
            'nick'
        );
        ...
        $lockItem->setData('nick','amazon_list_skus_queue_'.$this->marketplace->getId().'_'.$this->account->getId())
