...
class Ess_M2ePro_Model_Connector_Server_Buy_Product_Dispatcher
{
    const ACTION_LIST    = 1;
    ...
    public function process($action, $products, array $params = array())
    {
        ...
        if ($action == self::ACTION_LIST) {
            ...
            if (count($listProducts) > 0) {
                $sortedProductsListData = $this->sortProductsByAccountAndMarketplace($listProducts);
                $results[] = $this->processGroupedProducts($sortedProductsListData,
                                                           100,
                                                           'Ess_M2ePro_Model_Connector_Server_Buy_Product_List_Multiple',
                                                           $params);
            }

            if (count($results) <= 0) {
                $results[] = Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_ERROR;
            }

            return Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::getMainStatus($results);
        }

        $result = Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_ERROR;
        ...
        switch ($action) {
            case self::ACTION_RELIST:
                $result = $this->processGroupedProducts($sortedProductsData,
                                                        100,
                                                        'Ess_M2ePro_Model_Connector_Server_Buy_Product_Relist_Multiple',
                                                        $params);
                break;
            ...
            case self::ACTION_REVISE:
                $result = $this->processGroupedProducts($sortedProductsData,
                                                        100,
                                                        'Ess_M2ePro_Model_Connector_Server_Buy_Product_Revise_Multiple',
                                                        $params);
                break;
            ...
            case self::ACTION_STOP:
                $result = $this->processGroupedProducts($sortedProductsData,
                                                        100,
                                                        'Ess_M2ePro_Model_Connector_Server_Buy_Product_Stop_Multiple',
                                                        $params);
                break;
    ...
    protected function processProducts(array $products, $connectorName, array $params = array())
    {
        try {
            ...
            return Ess_M2ePro_Model_Connector_Server_Buy_Product_Requester::STATUS_ERROR;
        }
    }
