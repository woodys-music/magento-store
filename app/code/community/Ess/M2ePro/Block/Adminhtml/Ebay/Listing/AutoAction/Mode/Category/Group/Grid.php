    ...
    extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    public function __construct()
    {
        ...
        //------------------------------
        $this->setId('ebayListingAutoActionModeCategoryGroupGrid');
        //------------------------------
    ...
    protected function _prepareCollection()
    {
        // Get collection logs
        //--------------------------------
        $categoriesCollection = Mage::getModel('M2ePro/Ebay_Listing_Auto_Category')->getCollection();
        ...
        $categoriesCollection->getSelect()->where('melac.group_id = main_table.id');
    ...
    public function callbackColumnCategories($value, $row, $column, $isExport)
    {
        ...
        if ($count == 0 || $count > 3) {
            ...
            if (count($categories) > 3) {
                ...
                $html .= <<<HTML
&nbsp;
[<a href="javascript: void(0);" onclick="EbayListingAutoActionHandlerObj.categoryStepOne({$groupId});">{$details}</a>]
HTML;
            }
    ...
    public function callbackColumnActions($value, $row, $column, $isExport)
    {
        ...
        if (count($actions) == 1) {
            ...
            $onclick = 'EbayListingAutoActionHandlerObj[\''.$action['value'].'\']('.$id.');';
...
<div style="padding: 5px;">
    <select
        style="width: 100px;"
        onchange="EbayListingAutoActionHandlerObj[this.value]({$id});">
        {$optionsHtml}
    </select>
</div>
HTML;
