...
class Ranvi_Feed_Model_Item_Block extends Varien_Object
{
	protected $content = '';
	...
	public function setFeed(Ranvi_Feed_Model_Item $feed){
		$this->feed = $feed;
		return $this;
	}
    ...
    public function __construct($vars){
    	...
    	if($feed instanceof Ranvi_Feed_Model_Item){
        	...
        	throw new Exception('Feed must be instane of "Ranvi_Feed_Model_Item"');
        }
    }
    	...
    	while(preg_match($this->pattern, $this->content, $match_block)){
        	if(isset($match_block[2]) && ($content = trim($match_block[2]))){ /**/
        		...
        		*/
        		...
        		if(($type = $xml_block->getAttribute('type')) && ($modelClassName = Mage::getConfig()->getModelClassName('ranvi_feed/item_block_'.strtolower($type)))){
        			...
        			try{
	        			...
	        			if(false == ($block instanceof Ranvi_Feed_Model_Item_Block)){
	        	...
	        	if(is_null($block)){
	        		...
	        		$block = Mage::getModel('ranvi_feed/item_block', array('content'=>$content, 'feed'=>$this->feed, 'is_xml'=>1));
    ...
    public function render(){
    	...
    	return $this->setVars($this->content, null, (get_class($this) == 'Ranvi_Feed_Model_Item_Block'));
