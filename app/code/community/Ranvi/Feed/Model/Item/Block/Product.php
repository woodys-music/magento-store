...
class Ranvi_Feed_Model_Item_Block_Product extends Ranvi_Feed_Model_Item_Block{
		...
		public function writeTempFile($curr_page = 0, $length= 50, $filename = ''){
			...
			foreach($products as $_product){
				...
				$custom_attributes = Mage::getResourceModel('ranvi_feed/custom_attribute_collection');
			...
			for($i = 0;$i<$pages;$i++){
				...
				if ($_fp = fopen(Mage::getSingleton('core/store')->load($this->getFeed()->getStoreId())->getUrl('feed/index/index', array('id'=>$this->getFeed()->getId(), 'start'=>$i, 'length'=>$per_page, '_nosid' => true)), 'r')){
		            ...
		            if(!isset($response['result']) || !$response['result']){
		            	...
		            	throw new Mage_Core_Exception(Mage::helper('ranvi_feed')->__('There was an error while generating file. Change "Number of Products" in the Advanced Settings.
		        	...
		        	throw new Mage_Core_Exception(Mage::helper('ranvi_feed')->__('Cant open temp file'));
